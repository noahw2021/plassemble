...
#define Register(Source, Value, RegisterID, Mask) if (!strcmp(Source, "R" #RegisterID)) { *Value = atoi(Source); *Regmap |= Mask; } else { *Value = strtoull(Source, NULL, cgi_getinfo(_PT_UPDATEBASE)); } *PresentMap |= Mask;
